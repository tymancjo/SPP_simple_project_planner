...
function tasksUrl() {
	...
	if (!getit) {
		if (tasks.length) {
			...
			var urlData = JSON.stringify({ tasks: tasks, markedFW: markedFW });
			var serveradress = 'http://tomasztomanek.pl/pub/spp/api/';
			dataToServer(serveradress, urlData, getUID());
		}
	} else {
		// let thisUrl = window.location.href;  // grabbing the base adress
		// let url = new URL(thisUrl);
		var thisUrl = window.location.search; // grabbing the base adress
		var tasksString = '';
...
function dataFromServer(target, uid) {
	...
	$.get(target, function (data) {
		...
		if (data != 'false') {
			...
			var key = prompt('Please enter data coding key for this link.', 'secretkey');
			var decoded = false; // here we asuume it fail
